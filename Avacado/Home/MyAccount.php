...
$system = true;
$select = "UUID, PRNME AS Name, plldate, pel AS email, whoupdate";
$From = "tblpdu";
...
<table width="100%" border="0">
    <tbody>
        ...
        echo "<tr>";
        echo "<td>";
        		echo "<form method=\"post\" name=\"" . encryptfe($row['UUID']) . "\" action=\"?target=login&amp;section=reset\">\n\r";
                	$maths = rand();
                    $id = $row['UUID'] * $maths;
                   // confirm=$id&uid=$maths"
		echo "<input type=\"hidden\" name=\"confirm\" value=\"" . encryptfe($id) . "\" readonly=\"readonly\" />\n\r";
        echo "<input type=\"hidden\" name=\"uid\" value=\"" . encryptfe($maths) . "\" readonly=\"readonly\" />\n\r";
...
// no longer works after changes to encryption
// email address can no longer be used as the link to news letters
/*
